Page({
  ...
  onLoad: function() {
    ...
    this.downloadFile();
    this.drawGraph();
    const expanded = this.data.depsVtabs.map(() => false);
    this.setData({ expanded });
  },
  ...
  search: function() {
    ...
    const filteredData = this.data.depsVtabs.filter(item => {
      ...
      return item && item["河流（河段）"] && item["河流（河段）"].includes(searchKey);
    });
  ...
  downloadFile: function() {
    wx.cloud.downloadFile({
      fileID: 'cloud://gen-6g41ip5beb54cfa4.6765-gen-6g41ip5beb54cfa4-1344129462/new_file水质状况表.json',
      success: result => {
