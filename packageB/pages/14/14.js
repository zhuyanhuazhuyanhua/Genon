Page({
  ...
  downloadFile: function() {
    wx.cloud.downloadFile({
      fileID: 'cloud://gen-6g41ip5beb54cfa4.6765-gen-6g41ip5beb54cfa4-1344129462/new_file_full蓄积量-生物量模型数据库.json',
      success: result => {
        ...
        try {
          ...
          // 处理林分为 null 的情况
          let lastNonNull林分 = null;
          const processedData = data.map(item => {
            if (item['林分'] === null) {
              item['林分'] = lastNonNull林分;
            } else {
              lastNonNull林分 = item['林分'];
            }
            return item;
          });

          this.setData({
            depsVtabs: processedData
          }, () => {
